...
第一阶段以桥梁与道路巡检 AI Agent 的可审计业务闭环为边界，按以下五个里程碑交付；每个
里程碑都以前一阶段的权限、数据约束、迁移和验收证据为前置条件。

1. **基础边界**：安装并核验扩展，建立领域 Schema、身份、组织、项目和 RLS 基线，使每个
   后续事实都具备组织／项目边界与审计主体。
2. **空间资产与证据载体**：交付 Artifact、资产、构件、对象版本与 PostGIS 空间能力，形成
   可追溯的桥梁／道路资产定位和媒体证据引用。
3. **巡检事实与演化**：交付巡检、病害、量测和多期关联，区分稳定病害对象与每次观测，支持
   同一位置的演变分析。
4. **业务闭环与知识上下文**：完成第五章 Workflow 的兼容迁移，并接入 RAG、Memory、报告、
   复核签发和审计证据链。
5. **可运行验收**：完成 Outbox、派生索引、按需分区、迁移、备份恢复和真实项目验收；以
   PostgreSQL 权威事实与跨存储恢复证据作为发布门禁。

本阶段明确不纳入智慧工地业务域、湖仓／数据仓库、跨地域多主、pgvector 双写、未有容量
证据的全面分区，以及自动报告签发；这些能力如需引入，必须在后续章节或独立 ADR 中重新
定义边界、数据主权和验收合同。

### ADR-008-001 领域 Schema 与受控共享主数据

- **背景**：身份、项目、资产、巡检、工作流、知识和审计既需要清晰责任边界，也需要少量一致
  的组织、项目和代码类主数据。
- **决策**：按领域使用独立 Schema；共享主数据只经受控主表、外键和迁移入口提供，不以跨域
  复制表替代权威引用。
- **理由**：这使 owner、权限、迁移顺序和审计责任可定位，同时避免同一组织／项目事实在多个
  领域出现无控制的副本。
- **代价/约束**：查询需要显式 JOIN 与组合外键；任何新增共享数据必须指定唯一权威 owner，不能
  为便利绕开迁移或直接写入其他领域表。

### ADR-008-002 PostgreSQL 权威事实、MinIO 对象与 Qdrant 派生索引

- **背景**：业务状态、二进制证据和检索向量的生命周期、事务模型和容量特征不同。
- **决策**：PostgreSQL 保存权威业务事实与关系；MinIO 保存不可覆盖的对象版本；Qdrant 仅保存
  可由权威集重建的派生索引。
- **理由**：权威状态可由约束、RLS 和事务保护，对象版本可精确引用，向量索引故障不会成为业务
  事实丢失的单点。
- **代价/约束**：跨存储写入依赖 Outbox、状态机和补偿；Qdrant 或对象服务不可用时必须降级或
  拒读，不能把派生 payload 反写成权威事实。

### ADR-008-003 PostGIS 与 EPSG:4490 空间基线

- **背景**：桥梁、道路、构件、巡检位置和病害范围需要一致的地理定位与空间查询语义。
- **决策**：使用 PostGIS，第一阶段以 EPSG:4490 作为业务空间基线，并在约束和查询中验证 SRID。
- **理由**：统一坐标参考可让空间索引、距离／相交判断和跨期定位拥有可比较的语义。
- **代价/约束**：导入数据必须完成坐标转换和质量校验；不同精度、三维或工程局部坐标需求不得
  静默混入 EPSG:4490，须显式转换并保留来源。米制距离、缓冲和面积计算必须显式转换到项目
  适用的投影坐标系，并记录所用 SRID 与转换参数。

### ADR-008-004 组织／项目 RLS 与组合约束的共享库隔离

- **背景**：多组织、多项目数据在同一 PostgreSQL 集群中运行，单列 tenant 过滤容易在 JOIN 或
  外键处遗漏。
- **决策**：以组织／项目 RLS、受信任事务上下文和包含边界键的组合约束共同实施共享数据库隔离。
- **理由**：RLS 提供默认拒绝，组合键让引用在结构上不能跨 scope，二者共同覆盖应用路径和数据库
  完整性路径。
- **代价/约束**：角色、Policy、owner、`SET LOCAL` 上下文和查询计划都必须测试；不得用
  BYPASSRLS、关闭 RLS 或仅靠应用 WHERE 条件替代。

### ADR-008-005 稳定病害实体与单次观测分离

- **背景**：同一病害可在多个巡检期被重复识别，单次模型输出既可能修正也可能消失。
- **决策**：将可长期跟踪的病害实体与单次巡检观测、量测和关联证据分离建模。
- **理由**：稳定实体承载演化身份，观测保留当时算法、人员和证据，使变化趋势与误检复核可以
  重建。
- **代价/约束**：关联与拆并需要人工复核和可解释规则；不得以覆盖旧观测或重用单次检测 ID 的
  方式伪造连续性。

### ADR-008-006 正式结果采用不可变修订与引用快照

- **背景**：报告、复核结论和签发结果必须能够回答当时依据了哪一版数据、模型和证据。
- **决策**：正式结果使用不可变 revision，并保存 Artifact、病害、检索上下文和审批主体的引用
  快照。
- **理由**：引用快照可支持审计、复现和撤销后的历史解释，不会因 current/latest 指针变化而改写
  已签发证据。
- **代价/约束**：存储、保留和引用校验成本上升；修订只能前滚生成新版本，不能原地修改已正式
  使用的内容。

### ADR-008-007 事务 Outbox、状态机与补偿保证跨存储一致性

- **背景**：数据库提交、对象复制和派生索引写入无法依靠单一分布式事务原子完成。
- **决策**：权威写入与语义 Outbox 在同一数据库事务中提交；外部执行通过显式状态机、幂等键、
  重试与补偿收敛。
- **理由**：已提交的业务变化不会静默缺少待处理事件，失败可被观测、重放和审计，而非依赖隐式
  最终一致。
- **代价/约束**：Worker 必须处理重复、乱序、lease 和 dead-letter；补偿不得删除已受 legal hold、
  被签发引用或仍在保留期内的证据。

### ADR-008-008 第五章 Workflow 采用 expand-contract 兼容迁移

- **背景**：既有 Workflow 的任务、运行、Checkpoint 和事件需要迁入统一的组织／项目边界，且
  不能中断在途线程。
- **决策**：按 expand、backfill、verify、switch、contract 顺序实施兼容迁移，并以幂等和
  对账门禁控制切换。
- **理由**：新旧读写可在过渡期共存，数据质量、关联完整性和恢复路径可在删除旧结构前验证。
- **代价/约束**：过渡期需要双路径兼容代码和额外监控；未通过对账、RLS 或 Checkpoint 恢复验收
  时不得进入 contract 或清理旧字段。

### ADR-008-009 继续使用 Qdrant，不引入 pgvector 双写

- **背景**：第六、七章已以 Qdrant 的 payload 过滤、集合版本和别名承载 RAG 与 Memory 的派生
  检索能力。
- **决策**：第一阶段继续使用 Qdrant，PostgreSQL 保留可重建的权威 metadata；不引入 pgvector
  双写。
- **理由**：避免两套向量索引的一致性、成本和召回差异在项目验收前被放大，并保留已定义的重建
  与 ACL 过滤路径。
- **代价/约束**：Qdrant 故障期间必须按既定降级策略处理；未来替换需先做独立基准、差集验证和
  迁移 ADR，不能直接并行生产双写。

### ADR-008-010 高频事件按需分区，核心业务表不预分区

- **背景**：审计、事件和部分时序事实增长较快，但核心资产、项目和正式结果的规模尚无充分实测。
- **决策**：仅对有明确时间访问模式、保留策略和容量证据的高频表按月分区；核心业务表不预分区。
- **理由**：按需分区可改善保留与维护，同时避免过早分区带来的全局唯一、外键、迁移和查询复杂度。
- **代价/约束**：分区必须有 DEFAULT 护栏、未来边界和恢复测试；规模证据不足时不得为假设的未来
  规模拆分所有表。

## 8.28 本章结论

本章将桥梁与道路巡检 AI Agent 的权威事实落在具备组织／项目边界、组合完整性和审计主体的
PostgreSQL 关系模型中：资产、构件、巡检、病害、量测、报告和工作流均能追溯到明确的业务
身份。PostGIS 与 EPSG:4490 建立空间定位基线；稳定病害实体和单期观测分离，使多期演化、
复核纠偏和趋势判断不会覆盖历史证据。

正式报告和签发以不可变修订、引用快照、人工复核和审计链为前提。RAG 与 Memory 均从受控的
权威事实和 Artifact 版本派生，Qdrant 可重建且受 scope、ACL、版本和撤销状态约束。通过
事务 Outbox、状态机、补偿、备份恢复点、Manifest 和演练证据，本章将跨存储的一致性、恢复与
审计纳入发布门禁，而不把未验证的环境能力写成已达标事实。

第九章的 MCP 工具接入规范应以本章的权威数据边界、RLS 上下文、Artifact 精确版本、幂等、
审计和复核状态为输入：工具只能经受控入口读取或写入授权 scope，并把每次副作用、证据引用
和人工决策回写到可审计链路，不能绕过数据库约束或把派生检索结果提升为权威事实。

## 参考资料

以下资料均为 Task 1 已核验的官方来源，链接与适用性于 **2026-07-29** 核验；本章未新增
非官方参考资料。
...
| 版本 | 日期 | 修订说明 |
|---|---|---|
| V1.0 | 2026-07-29 | 正式发布《第八章：数据与数据库设计》：覆盖领域数据边界、PostgreSQL/PostGIS 设计、RLS、Artifact 与派生存储、巡检病害与多期演化、Workflow/RAG/Memory、报告审计、迁移分区、备份恢复、性能验收、第一阶段范围与 ADR；真实环境未执行项仍按 GAP 管理 |
